...
- `wilson-resume.yml` — create empty scaffold if it doesn't exist:
  ```yaml
  header:
    name: ""
    title: ""
...
|------|-------------|------------|
| `wilson-resume.yml` | Single source of truth for all resume content | /setup |
...
| File | Created By |
|------|------------|
| `wilson-resume.md`, `wilson-resume.pdf`, `wilson-resume.docx`, `wilson-resume.html` | /generate |
| `knowledge/sessions/{company}/{role}/tailored/` (wilson-resume.yml, PDF, DOCX, HTML, MD, cover letter) | /tailor, /cover-letter, /apply |
...
| Pattern | Why |
|---------|-----|
| `skills.yaml` | Skill data belongs in `wilson-resume.yml` under `skills:` |
...
### /import
Import an existing resume into resume-builder format. Scans for existing resume files (PDF, DOCX, Markdown, YAML), interviews the user to fill gaps, writes `wilson-resume.yml`.

### /generate
Generate PDF, DOCX, HTML, and Markdown from `wilson-resume.yml`. Fully deterministic, no LLM involvement in rendering. Supports template customization via `templates/` directory.
...
### /research
Research a company and build a CompanyProfile — pain points, tech stack, culture, growth signals, key people. Every fact traced to source. Profiles persist in `knowledge/sessions/{company}/company.yaml` for reuse. Use before /tailor, /qualify, /review, or /cover-letter for sharper results.
...
### /cover-letter
Generate a tailored cover letter (250-400 words) with claim verification. Every factual claim must trace to `wilson-resume.yml`. Consumes `/match`, `/qualify`, and CompanyProfile data when available for stronger positioning. Saves to `knowledge/sessions/{company}/{role}/tailored/cover_letter.md`.

### /apply
End-to-end application pipeline. Fire-and-forget: runs research → match → qualify → tailor → score → review → cover-letter → verify in one shot. Produces tailored resume (PDF/DOCX/HTML/MD), cover letter, scores, and review. Optimized for multi-job-same-company use — company research is performed once and reused. All output lands in `knowledge/sessions/{company}/{role}/tailored/`.

## How Skills Connect

```
setup ──→ project structure + plugin docs
import ──→ wilson-resume.yml ──→ generate (outputs)
               │
               ├──→ ingest ──→ verify (auto)
               │
...
|----------|------------|--------|------|
| First-Time Setup | Just installed, no wilson-resume.yml | setup, import, generate | 15-30 min |
...
| Tool | Description |
|------|-------------|
| `generate` | Generate resume outputs from wilson-resume.yml |
...
Next: /import to migrate an existing resume, or start building wilson-resume.yml manually.
Re-run /setup after plugin updates to refresh docs.
```

## Rules
- NEVER overwrite user content in wilson-resume.yml if it already has data
