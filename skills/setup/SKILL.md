...
### /cover-letter
Generate a tailored cover letter (250-400 words) with claim verification. Every factual claim must trace to `resume.yaml`. Consumes `/match`, `/qualify`, and CompanyProfile data when available for stronger positioning. Saves to `knowledge/sessions/{company}/{role}/tailored/cover_letter.md`.
